...
namespace JWWS\Admin_Columns_Add_On\Modules\Columns\Categories_Hierarchy\Column\Free;
...
class Root extends \AC\Column {
    ...
    public function __construct() {
        $this
            ->set_type(type: 'column-categories_hierarchy')  // Identifier, pick an unique name. Single word, no spaces. Underscores allowed.
            ->set_group(group: 'jwws-woocommerce')
            ->set_label(label: __('Categories (Hierarchy)', 'jwws')) // Default column label.
        ;
    }
    ...
    public function get_raw_value(mixed $post_id): mixed {
        ...
        foreach ($categories as $category) {
            ...
            if ($ancestor_ids) {
                $all_categories = $this->get_ancestor_names(ancestor_ids: $ancestor_ids);
            }
